...
const Descriptions: React.FC<InnerDescriptionsProps> = (props) => {
  ...
  const descriptionKls = classnames('el-descriptions',
    `is-el-descriptions--${size}`)
  const state = useMemo(() => ({ direction, border }), [border, direction])
  return (<DescriptionsContext.Provider value={state}>
      <div className={descriptionKls} style={props.style}>
        {(title || extra) && <div className="el-descriptions__header">
        ...
        <div className="el-descriptions__body">
          <table className={classnames('el-descriptions__table',
            { 'is-bordered': border })}>
            <tbody>
            {getRows.map(
              (row, index) => <DescriptionsRow row={row} key={index}/>)}
            </tbody>
          </table>
        </div>
      </div>
