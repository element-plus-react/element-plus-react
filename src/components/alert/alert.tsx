...
const Alert: React.FC<AlertProps> = (props) => {
  ...
  const isBigIcon = useMemo(
    () => props.description || props.children ? 'is-big' : '', [props.children, props.description])
  const isBoldTitle = useMemo(
    () => props.description || props.children ? 'is-bold' : '', [props.children, props.description])
  // methods
  ...
  return (
    ...
    <div style={!visible ? { display: 'none' } : undefined}
      ...
      {props.showIcon && iconComponent && <i className={iconClassNames}>{iconComponent}</i>}
      <div className="el-alert__content">
        {props.title && <span className={titleClassNames}>{props.title}</span>}
        {props.children || props.description &&
          <p className="el-alert__description">{props.description}</p>}
        {closable && <>{props.closeText ? <div className="el-alert__closebtn is-customed"
