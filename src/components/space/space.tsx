...
const Space: React.FC<SpaceProps> = (props) => {
  ...
  if (isArray(props.children)) {
    ...
    props.children.forEach((child, loopKey) => {
      extractedChildren.push(<SpaceItem style={itemStyle} prefixCls={prefixCls}
                                        key={`LoopKey${loopKey}`}>{child}</SpaceItem>)
    })
      ...
      extractedChildren = extractedChildren.reduce<React.ReactNode[]>((acc, child, idx) => {
          ...
          if (idx !== len) {
            ...
            if (direction === 'vertical') {
             ...
             children.push(<span style={style} key={idx}>{spacer}</span>)
          }
          return children
        }, [])
    }
    ...
    return (<div className={classes} style={containerStyle}>{extractedChildren}</div>)
  }
