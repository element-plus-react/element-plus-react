...
const Step: React.FC<StepProps> = (props: StepProps & ParentTypes) => {
  ...
  const currentStatus = useMemo(() => props.status || internalStatus,
  ...
  const calcProgress = useCallback((status) => {
    let step = 100
    const innerStyle: Record<string, unknown> = {}
  ...
  const updateStatus = useCallback((activeIndex) => {
  ...
  }, [props.index, prevStatus, parent.steps, parent.props.finishStatus, parent.props.processStatus, stepsCount, calcProgress, internalStatus])
  ...
  return (
    <div style={style} className={classNames}>
      ...
      <div className={`el-step__head is-${currentStatus}`}>
        {!isSimple && <div className="el-step__line">
        ...
        <div className={`el-step__icon is-${icon ? 'icon' : 'text'}`}>
          {currentStatus !== 'success' && currentStatus !== 'error' ? <>
            ...
            {!icon && !isSimple && <div className="el-step__icon-inner">
              {props.index + 1}
            </div>}
          </> : <i className="el-step__icon-inner is-status">
            {currentStatus === 'success' ? <CheckCircleOutlined/> :
              <CloseCircleOutlined/>}
          </i>}
        </div>
      </div>
      ...
      <div className="el-step__main">
        <div className={`el-step__title is-${currentStatus}`}>
          {props.title}
        </div>
        {isSimple ? <div className="el-step__arrow"/> : <div
          className={`'el-step__description is-${currentStatus}`}>
          {props.description}
        </div>}
      </div>
    </div>
